...
Builder.load_file("kv/labels_window.kv")
Builder.load_file("kv/io_controll.kv")
...
#if config['relay_board1']['i2c_bus'] != '-1':
...
class IO_controll_group(Scatter):
    def io_swich(self, arg , ch):
        print(ch)
        global io_status
        if arg == "True":
            io_status = self.set_bit(io_status, int(ch), 0)
            bus.write_byte_data(0x20, 0x02, io_status)
            #bus.write_byte_data(0x20, 0x03, io_status)
            #bus.write_word_data(0x21, 0x02, io_status)

            #controller_handler.command_write(self, "bit:" + ch + ":1:s")
        if arg == "False":
            io_status = self.set_bit(io_status, int(ch), 1)
            bus.write_byte_data(0x20, 0x02, io_status)
            #bus.write_byte_data(0x20, 0x03, io_status)
            #bus.write_word_data(0x21, 0x02, io_status)


            #controller_handler.command_write(self, "bit:" + ch + ":0:s")

    def IO_reset(self):
        controller_handler.command_write(self, "bit:rst:1:s")

    def timer_on(self, arg, ch):
        global timer_en
        if arg == "True":
            timer_en = 'true'
        if arg == "False":
...
class Labels_group(Screen):

    pass


class PopUpWindow(Screen):
    text = StringProperty('hh')
    def popupVent(self):

        print("popup ACT <<<<<<<<<<<<<<")
        self.print_result(data_for_print)
    def on_open(self):
...
class Meas_lbl_group(Screen):
    ...
    InputWaterPressure = StringProperty('[color=FFFFFF]WIP[/color]')
    OutputWaterPressure = StringProperty('[color=FFFFFF]WOP[/color]')
    OutputWaterPressure2 = StringProperty('[color=FFFFFF]WOP2[/color]')
...
class btns_group(Scatter):
    lbl_1_1 = StringProperty(None)
    stuff_p = StringProperty(None)


    def fn(self,arg):
        print(arg)
        self.ids.lbl_1_1.text = arg
        Dashboard.data_to_graph = []
        Dashboard.sec_tick = int(0)
    ...
    def refresh(self , arg):
        print("action")
        #self.ids.lbl_1.text = self.lbl_1
...
class Dashboard(FloatLayout):
    ...
    def __init__(self, **kwargs):
        ...
        self.show = PopUpWindow()  # Create a new instance of the P class
        self.popupWindow = Factory.Popup(title="Information field", content=self.show, size_hint=(None, None), size=(400, 300))

        self.SysInfoLabel = Button(markup=True, font_size=44, text='[color=000000]MK-40/300[/color]', pos=(60, 910),
                              size=(335, 50), size_hint=(None, None), background_normal='images/SysInfoLBL.png',
        ...
        self.AnimWaterFlow = Image(source='images/AnimWaterFlow.gif', opacity=0, size_hint=(.2, .2), pos=(self.air_anim_x-80, self.air_anim_y-37), anim_loop=0, anim_delay=-1)
        ...
        #self.player = Video(source='images/Cheap.mp4', options={'eos': 'loop'}, size=(750, 400), size_hint=(None, None), pos= (85,450))
        ...
        #DEBUG SECTION<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
        #self.lbl_group = Labels_group(pos=(10, 600), size=(300, 250), size_hint=(None, None))
        #self.IO_group = IO_controll_group(pos=(390, 480), size=(300, 350), size_hint=(None, None))
        #self.add_widget(self.IO_group)
        #self.add_widget(self.lbl_group)
    ...
    def screen_update(self):
        ...
        if adc2_arr[0] == "S0" and adc2_arr[1] == "run" and len(adc2_arr) == 8:
            ...
            self.Meas_lbl.AirTemperature = str(ntc_table[int(adc2_arr[3])]/10)
            ...
            if self.current_operation == "post_check" and self.timer_en == "true":
                ...
                if self.T_iteration >= 100:
                    ...
                    self.show.text = datetime.now().strftime('%d/%m/%Y-%H:%M:%S')  + '\n'
                    self.show.text += 'Average after precheck: ' + str(pre_check_average) + ' KPa\n'
                    self.show.text+= 'Average after cleaning: ' + str(post_check_average) + ' KPa\n'
                    self.show.text += "Cleaning time:" + str(CleaningTimeInSecond) + ' Seconds\n'
                    self.show.text += "Drying time:" + str(DryingTimeInSecond) + ' Seconds\n'
                    #self.print_result(self.show.text)
                    data_for_print = self.show.text
                    try:
                        # with open('/dev/ttyUSB0', "w") as f:
                        with open('/home/vanya/PycharmProjects/Matsola_GUI/logs/log.txt', "a") as f:
                            f.write(self.show.text + '\n printer ready \n\r\x1b\x69')
                            f.write('\x0c\n')
                            print("printer OK")
                    except:
                        print("An exception occurred")
    ...
    def one_second_loop(self):
        ...
        if self.timer_en == 'true':
            ...
            if second_duration == 60:
                minuts_duration += 1
        self.Meas_lbl.run_time = str(minuts_duration).zfill(2) + ":" + str(second_duration).zfill(2)


        if heater1 == 'true' and (ntc_table[int(adc2_arr[4])] / 10) <= 60:
            self.set_uot(3, 1)
        else:
            self.set_uot(3, 0)
        if heater2 == 'true' and (ntc_table[int(adc2_arr[4])] / 10) <= 60:
            self.set_uot(4, 1)
        else:
            self.set_uot(4, 0)
...
if __name__ == '__main__':
    ...
    os.system('stty -F ' + controller_dev + ' ' + controller_baud + ' cs8 -cstopb -parenb -echo')
    os.system('stty -F ' + adc_board_dev + ' ' + adc_board_baud + ' cs8 -cstopb -parenb -echo')
